...
class QuestionPaser: # 问题解析类

    '''构建实体节点'''
    def build_entitydict(self, args): # 构建实体字典
        entity_dict = {}
        for arg, types in args.items(): # 遍历args字典
            for type in types: # 遍历types列表
                if type not in entity_dict: # 如果type不在entity_dict中
                    entity_dict[type] = [arg ] # 将type作为key，arg作为value，构建字典
                else: # 如果type在entity_dict中
                    entity_dict[type].append(arg) # 将arg添加到value列表中

        return entity_dict

    '''解析主函数'''
    def parser_main(self, res_classify): # 解析主函数, res_classify是问题分类的结果
        ...
        for question_type in question_types:
            ...
            if question_type == 'disease_symptom':
                sql = self.sql_transfer(question_type, entity_dict.get('disease')) # 调用sql_transfer函数，得到sql语句
    ...
    '''针对不同的问题，分开进行处理'''
    def sql_transfer(self, question_type, entities): # sql语句转换，question_type是问题类型，entities是实体列表
        if not entities:
            return []
